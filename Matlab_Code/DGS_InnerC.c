...
void testMex(mwSize N, double *inU, double *inV, double *inP, double *outU, double *outV,double *outP)
{
    mwSize i,j;
    double r,delta,h;
    h = 1./((double)N);
    ...
    for (i=1; i<N; i++) {
        for (j=1; j<N+1;j++){
            outU[U_cor(i,j,N)] = inU[U_cor(i,j,N)];
        }
    }
    for (i=1; i<N+1; i++) {
        for (j=1; j<N;j++){
            outV[V_cor(i,j,N)] = inV[V_cor(i,j,N)];
        }
    }
    for (i=1; i<N+1; i++) {
        for (j=1; j<N+1; j++){
            outP[P_cor(i,j,N)] = inP[P_cor(i,j,N)];
        }
    }
    for (i=2; i<N; i++){
        for (j=2; j<N; j++){
                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    testMex((mwSize)N, inU, inV, inP, outU, outV, outP);
}
